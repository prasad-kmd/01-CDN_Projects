document.addEventListener("DOMContentLoaded", () => {
    ...
    const themeTransitionObserver = document.getElementById("theme-transition-observer");
        ...
        const observer = new MutationObserver((mutations) => {
            mutations.forEach((mutation) => {
                if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                    ...
                    document.documentElement.style.setProperty('--theme-transition-trigger', isDarkTheme ? '1' : '0');
    ...
    document.addEventListener("click", (event) => {
        ...
        if (!isClickInsideMenu && !isClickOnToggle && !isClickOnThemeToggle && navMenu.classList.contains("active")) {
    ...
    function toggleTheme(event) {
        ...
        if (event && event.type === 'change') {
            ...
            if (event.target === footerThemeToggle) {
                if (footerThemeToggle.checked && !body.classList.contains('dark-theme')) {
                    body.classList.add('dark-theme');
                } else if (!footerThemeToggle.checked && body.classList.contains('dark-theme')) {
                    body.classList.remove('dark-theme');
                }
                
                // Save theme preference to localStorage
                localStorage.setItem("theme", footerThemeToggle.checked ? "dark" : "light");
    ...
    function testFooterThemeAdaptation() {
        ...
        const footerHeading = getComputedStyle(document.querySelector('.footer-column h3')).color;
        const footerLink = getComputedStyle(document.querySelector('.footer-links a')).color;
        const footerBorder = getComputedStyle(document.querySelector('.footer-bottom')).borderTopColor;
